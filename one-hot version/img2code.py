...
class img2code(object):
  ...
  #output_size, the dimension of the embedding vector or vocabulary size if 
  #one-hot encoding. The hidden state size of LSTM are also set equal to
  #output_size
  ...
  def __init__(self, input_shape, output_size, output_path, name = "img2code"):
    ...
    token_encoder.add(
      LSTM(units = 128, activation = 'tanh', recurrent_activation = 'hard_sigmoid',
        ...
        stateful = False, input_shape = (CONTEXT_LENGTH, output_size))
      )
    token_encoder.add(
      LSTM(units = 128, activation = 'tanh', recurrent_activation = 'hard_sigmoid',
        dropout = 0.0, recurrent_dropout = 0.0, implementation = 2, return_sequences = True,
    ...
    token_input = Input(shape = (CONTEXT_LENGTH, output_size))
