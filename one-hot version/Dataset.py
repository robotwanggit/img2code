...
class Dataset(object):
  ...
  #output_size is the dimension of the embedding vector or vocabulary size when using
  #one-hot encoding. The hidden state size of LSTM are also set equal to output_size
  ...
  def load(self, path, generate_binary_sequences = False):
    ...
    #convert self.token_segments from format string to format index or one-hot encoding
    ...
    #only vaid for one-hot encoding, for word embedding, output_size equals to embedding dimension
